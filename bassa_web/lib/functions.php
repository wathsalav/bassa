...
function insert_file($email,$file,$url,$ip,$size){
  ...
  if ($file!=''){
	...
	if($fp) {
	  ...
	  echo "Your request had been send to BASSA for download.\n";
	  ...
	  echo "The Download request pending.\n";
	}
	$_SESSION['size']='';
      }
    }
    /*add file in to user account*/
    $didquery = mysql_query("SELECT id FROM downloads where url='$b_url'");
    $did = mysql_result($didquery, 0, id);
    
    mysql_query("INSERT INTO userdown (user_id,down_id,file_id) VALUES('$email','$did','$file')") or die (mysql_error());
...
function list_downloads($result){
  ...
  while ($i<$num) { 
    ...
    $my_cache=mysql_result($result,$i,local_url);
    //size in Meg 
    ...
    if ($my_status==completed){
      $data.="<td><a href='$my_cache'><img src='images/download.gif'></a></td>";	
    }
    if($my_status==queue){
      $data.="<td>On Queue</td>";
    }
    ...
    $data.="<td><a href='http://$server/gadisk/info.php?id=$my_id'><img src='images/info.png'></a></td>";	
