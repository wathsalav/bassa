...
if (strstr($file,'.flv')||strstr($file,'.swf')){
...
<?
	...
	echo '<tr><th colspan=2>File Details</th></tr>';
		...
		if ($email==$admin){
                ...
                if ($status_1==completed){ echo " <tr><td>Click here to Download </td><td><a href='$ftp_host/$file'>$file</a></td></tr/>";}
                if ($status_1==failed && ($email==$user ||$email==$admin)){
                echo "<tr><td>If you want to retry this download, please click here </td> ";
                echo "<td><a href='http://cache.cmb.ac.lk/gadisk/action.php?action=ResendBassa&url=$url&size=$size'><img src='images/retry.jpg'></a></td></tr>";
                }
