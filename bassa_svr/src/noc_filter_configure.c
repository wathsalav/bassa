...
bassa_start_tag_handler(void *udata, char *name, char **attr)
{
  ...
  else if (!strcmp(name, REPOSITORY_URL_TAG))
    conf->current_tag = REPOSITORY_URL_ID;
  ...
  if (!strcmp(name, DBI_TAG))
    {
      conf->current_tag = DBI_ID;
      conf->current_config_module = DBI_CONF_MODULE;
      conf->dbicfg = bassa_dbi_conf_new();
    }
  else if (!strcmp(name, DBI_DBMS_TAG))
    conf->current_tag = DBI_DBMS_ID;
  else if (!strcmp(name, DBI_DBNAME_TAG))
    conf->current_tag = DBI_DBNAME_ID;
  else if (!strcmp(name, DBI_DBHOST_TAG))
    conf->current_tag = DBI_DBHOST_ID;
  else if (!strcmp(name, DBI_DBPORT_TAG))
    conf->current_tag = DBI_DBPORT_ID;
  else if (!strcmp(name, DBI_DBUSER_TAG))
    conf->current_tag = DBI_DBUSER_ID;
  else if (!strcmp(name, DBI_DBPASSWD_TAG))
    conf->current_tag = DBI_DBPASSWD_ID;
}
...
bassa_setup_server_configuration (bassa_conf *conf, char *s, int len)
{
  if (conf->current_tag == SERVER_ADDRESS_ID)
   ...
   else if (conf->current_tag == SERVER_URL_ID)
    {
      char* str_url = NULL;	
...
void
bassa_setup_dbi_configuration (bassa_conf *conf, char *s, int len)
{
  if (conf->current_tag == DBI_DBMS_ID)
    {
      conf->dbicfg->dbms =
        bassa_assemble_configuration (conf->dbicfg->dbms, s, len);
    }
  else if (conf->current_tag == DBI_DBNAME_ID)
    {
      conf->dbicfg->db_name =
        bassa_assemble_configuration (conf->dbicfg->db_name, s, len);
    }
  else if (conf->current_tag == DBI_DBHOST_ID)
    {
      conf->dbicfg->db_host =
        bassa_assemble_configuration (conf->dbicfg->db_host, s, len);
    }
  else if (conf->current_tag == DBI_DBPORT_ID)
    {
      char *dbport_str = NULL;
      dbport_str = bassa_assemble_configuration (dbport_str, s, len);
      conf->dbicfg->db_port = atoi(dbport_str);
      if (dbport_str)
        free (dbport_str);
    }
  else if (conf->current_tag == DBI_DBUSER_ID)
    {
      conf->dbicfg->db_user =
        bassa_assemble_configuration (conf->dbicfg->db_user, s, len);
    }
  else if (conf->current_tag == DBI_DBPASSWD_ID)
    {
      conf->dbicfg->db_passwd =
        bassa_assemble_configuration (conf->dbicfg->db_passwd, s, len);
    }
}
...
bassa_dbi_conf_new()
{
  bassa_dbi_conf *conf = (bassa_dbi_conf*)malloc(sizeof(bassa_dbi_conf));
  return conf;
}
...
bassa_parse_configuration (char *file_name)
{
  ...
  bassa_conf *lconf = bassa_conf_new ();
  XML_Parser parser = XML_ParserCreate (NULL);
  XML_SetUserData (parser, (void *) lconf);
  ...
  free (xml_buffer);
  if (lconf->svrcfg->http_proxy_address)
    {
      int proxy_len = strlen(HTTP_PROTO_PREFIX) + strlen(lconf->svrcfg->http_proxy_address) + 6;\
      char *proxy = (char*)malloc(proxy_len);
      memset (proxy, (int)'\0', proxy_len);
      sprintf (proxy, "%s%s:%i", HTTP_PROTO_PREFIX, lconf->svrcfg->http_proxy_address, lconf->svrcfg->http_proxy_port);\
      lconf->svrcfg->http_proxy = proxy;
    }	
  return lconf;
}
